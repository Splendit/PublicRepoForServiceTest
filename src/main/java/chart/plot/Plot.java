...
public abstract class Plot implements DatasetChangeListener, PublicCloneable, Cloneable, Serializable {
	 ...
	 *
	 * @param alpha
	 *            the alpha transparency (in the range 0.0f to 1.0f, where 0.0f is
	...
	public boolean equals(Object obj) {
		...
		if (!ObjectUtils.equal(this.noDataMessageFont, that.noDataMessageFont)) {
			return false;
		}
		...
		if (!ObjectUtils.equal(this.backgroundPainter, that.backgroundPainter)) {
			return false;
		}
		if (!ObjectUtils.equal(this.backgroundImage, that.backgroundImage)) {
			return false;
		}
