...
public class NodeReplaceInsert extends BaseHandler {
	...
	private static String ceilingEntity = "ceilingEntity";
	...
	protected boolean replaceCeilingEntityNode(Node[] primaryNodes, Node testNode, List<Node> usedNodes) {
		...
		if (testNode.getAttributes().getNamedItem(ceilingEntity) == null) {
			return false;
		}

		Node[] filtered = NodeUtil.filterByAttribute(primaryNodes, ceilingEntity);

		int pos = NodeUtil.findNode(filtered, testNode, ceilingEntity, true);
